...
//     func fetchData() {
//     db.collection("money").addSnapshotListener { (querySnapshot, error) in
...
//           return TransactionModel(id: .init(), date: date, title: title, details: details, expense: amount, type: true)
//       }
//     }
//   }
...
func fetchData() {
    ...
    db.collection("money").getDocuments { snapshot, error in
    ...
    if let snapshot = snapshot{
        for documents in snapshot.documents{
          ...
          let date = data["date"] as? String ?? ""
          let type = data["type"] as? String ?? ""
          let tmp = TransactionModel(id: .init(), date: date, title: title, details: details, expense: amount, type: type)
          self.transcations.append(tmp)
        }
    }
